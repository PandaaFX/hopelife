...
--added requiredItems by DaBurnerGermany
RegisterServerEvent('myFarming:collectItems')
AddEventHandler('myFarming:collectItems', function(item, label, amount, xptype, requiredItems)
    ...
    --remove RequiredItem if Config says to remove
    if requiredItems~= nil then
        if #requiredItems > 0 then

            for i=1, #requiredItems do
                if requiredItems[i].removeAfterCollect then 
                    xPlayer.removeInventoryItem(requiredItems[i].name, requiredItems[i].amount)
                    TriggerClientEvent('myFarming:msg', _source,  Translation[Config.Locale]['removed_item'] .. requiredItems[i].label)
...
AddEventHandler('myFarming:craftItem', function(itemData)
    ...
    if itemData.requiredItemNotRemove == nil or xPlayer.getInventoryItem(itemData.requiredItemNotRemove).count > 0 then
...
AddEventHandler('myFarming:finishedCrafting', function(itemData, craftAmount)
    ...
    for k, v in pairs(itemData.reqItems) do
        ...
        if k == #itemData.reqItems then
            ...
            if itemData.randomchance == nil then
            ...
            if itemData.randomchance ~=nil then
                if chance >= itemData.randomchance then
                    xPlayer.addInventoryItem(itemData.newitem.sqlitem, itemData.newitemamount * craftAmount)
                    TriggerClientEvent('myFarming:msg', _source, Translation[Config.Locale]['craftting_finished'] .. itemData.newitemamount * craftAmount .. 'x ' .. itemData.newitem.label .. Translation[Config.Locale]['craftting_finished2'])
                else
                    TriggerClientEvent('myFarming:msg', _source, "Nichts gefunden")
                end
            end
...
AddEventHandler('myFarming:sellItems', function(item, amount, price)
    ...
    if amount > 0 and amount < 99999 and itemCount > 0 and itemCount >= amount then
        --print('test')
...
RegisterServerEvent('myFarming:buyItems')
AddEventHandler('myFarming:buyItems', function(item, amount, price)
    local _source   = source
    local xPlayer   = ESX.GetPlayerFromId(_source)
    local job     = xPlayer.job.grade_name
    local moneycount = xPlayer.getMoney()
    local tax = price * amount
    TriggerEvent('esx_society:getSociety', xPlayer.job.name, function (society)
        if society ~= nil then 
            TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function (account)
                if account.money >=  exports['cyber_Tax']:CalculateTax(tax) then
                    xPlayer.addInventoryItem(item, amount)
                    --account.removeMoney(tax)
...
function setRandomAssignments()
    if #shopContent == 0 then
        if #availableAssignments > 0 then
            ...
            --print('myFarming: You removed all missions from the Config, so this feature is disabled.')
        end
...
--added by DaBurnerGermany
ESX.RegisterServerCallback('myFarming:checkRequiredItems', function(source, cb, requiredItems)
    local xPlayer = ESX.GetPlayerFromId(source)
    local retval = false
    local missingItems = ''
    ...
    if requiredItems~= nil then
        if #requiredItems > 0 then
            retval = true
            for i=1, #requiredItems do
                if xPlayer.getInventoryItem(requiredItems[i].name).count < requiredItems[i].amount then
                    retval = false
                    if missingItems ~= '' then missingItems = missingItems .. ', '  end 
                    missingItems = missingItems .. requiredItems[i].label
                end
            end
        else
            retval = true
