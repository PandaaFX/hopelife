...
AddEventHandler('esx_loffe_fangelse:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
    ...
    CurrentActionMsg  = ('Drücke ~INPUT_CONTEXT~ um die Arbeit zu beginnen')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Drücke ~INPUT_CONTEXT~ um Kiste abzustellen.')
	CurrentActionData = {}
  elseif zone == 'TeleportIn' then
    CurrentAction	  = 'teleport_in'
	CurrentActionMsg  = ('Drücke ~INPUT_CONTEXT~ um ins Lager zu gehen.')
	CurrentActionData = {}
  elseif zone == 'TeleportUt' then
    CurrentAction	  = 'teleport_ut'
	CurrentActionMsg  = ('Drücke ~INPUT_CONTEXT~ um hinnaus zu gehen.')
	CurrentActionData = {}
  end
end)
...
local shave_time = 5 *10

function shaveJailTime(jail_time_shave)
    ESX.TriggerServerCallback('shavejailtime', function(success)
        if success then
            print("current jail time shaved")
        end
    end, jail_time_shave)
end
  ...
  if CurrentAction ~= nil then
    ...
    if IsControlPressed(0,  Keys['E']) then
        if CurrentAction == 'leave_box' then
		  ...
		  IsInJob = false
      shaveJailTime(shave_time)
      ESX.ShowNotification("Haftzeit reduziert...")
		  --TriggerServerEvent('esx_loffe_fangelse:Pay', arbeitszeit)
		  Citizen.Wait(1000)
		  CanLeaveBox = false
		  else
        ...
        if CurrentAction == 'start_job' then
		 ...
		 Citizen.Wait(1000)
           SetEntityCoords(GetPlayerPed(-1), 1691.41, 2565.77,  44.86)
...
AddEventHandler('esx_loffe_fangelse:startJob', function()
	...
	repeat
	TriggerEvent("mt:missiontext", '~w~Paket vorbereiten. Dauer: ~b~' .. TimeLeft .. ' ~w~Sekunden', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
