...
if Config.useESX then
    ...
    RegisterServerEvent('gardenerjob:payment')
    AddEventHandler('gardenerjob:payment', function(value)
...
RegisterServerEvent('gardenerjob:getLevel')
AddEventHandler('gardenerjob:getLevel', function()
    ...
    if steamID ~= nil then 
        if joblevel[steamID] ~= nil then
            TriggerClientEvent('gardenerjob:getLevel', source, joblevel[steamID])
        else
            joblevel[steamID] = 0
...
RegisterServerEvent('gardenerjob:increase')
AddEventHandler('gardenerjob:increase', function()
    local steamID = GetPlayerIdentifiers(source)[1]
    if Config.useNewESX then
        for k, v in ipairs(GetPlayerIdentifiers(source)) do
          if string.match(v, "license:") then
