...
-- RegisterCommand("128937udjakgshjez", function(source)
...
--[[Citizen.CreateThread(function()
    while true do
        ...
        if (GetDistanceBetweenCoords(playerCoords, -1112.782, -2892.475, 16.2887, false) < 2) then
            ESX.ShowHelpNotification('Drücke ~INPUT_CONTEXT~ um deinen Char anzupassen')
            if IsControlJustPressed(0, 38) then
                TriggerEvent('VexCreator:loadCreator', playerPed)
            end
        else
            Citizen.Wait(500)
        end
...
RegisterNUICallback('selectedspawn', function(data)
    ...
    if location == "Airport" then
        ...
        DoScreenFadeOut(800)
        --Citizen.Wait(1000)
        SetEntityCoords(ped, playerCoords.x, playerCoords.y, playerCoords.z +4)
		DoScreenFadeIn(800)
        FreezeEntityPosition(ped, false)
        SetPlayerRoutingBucket(PlayerPedId(), 0)
    end
end)
...
AddEventHandler('VexCreator:loadCreator', function()
	...
	isOnCreator = true
	VexCreator(true)
    --SetPlayerRoutingBucket(playerPed, math.random(1,200))
end)
