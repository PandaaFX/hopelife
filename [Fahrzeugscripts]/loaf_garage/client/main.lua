...
AddEventHandler('esx:setJob', function(job)
  ...
  for k, v in pairs(Config.Garages) do
        RemoveBlip(Cache["blip_garage_"..k])
            -- print(PlayerData.job.name)
        if PlayerData.job.name == v.specifiedJob then
            -- print(PlayerData.job.name)
            Blips[k] = AddBlip(v.browse.xyz, 50, 38, Strings["frakgarage"])
        elseif v.specifiedJob == nil then
            Blips[k] = AddBlip(v.browse.xyz, 50, 38, Strings["garage"])
        end
    Wait(5)
end
end)
    ...
    ViewVehicles = function(garage, coords, heading, tpback)
        ...
        ESX.TriggerServerCallback("loaf_garage:getVehicles", function(vehicles)
            if vehicles and #vehicles > 0 then
                ...
                local dataNUI = vehicles[current_vehicle]
                local infoNUI = json.decode(dataNUI.vehicle)
                local modelNUI = infoNUI.model
                SendNUIMessage({ action = "newveh", vehname = GetDisplayNameFromVehicleModel(modelNUI), vehdisplayname = GetLabelText(GetDisplayNameFromVehicleModel(modelNUI)), plate = dataNUI.plate, garage_name = garage, vehicleData = {EstimatedMaxSpeed = round(GetVehicleModelEstimatedMaxSpeed(modelNUI) * 3.6,1), Acceleration = round(GetVehicleModelAcceleration(modelNUI) * 100, 1), MaxBraking = round(GetVehicleModelMaxBraking(modelNUI) * 10,1), MaxTraction = round(GetVehicleModelMaxTraction(modelNUI) * 10,1)}})
                ...
                while true do
                    ...
                    if vehicles[current_vehicle].stored == 0 or vehicles[current_vehicle].stored == false then
                        HelpText(string.format(Strings["browsing_vehicles"]))
                        SendNUIMessage({ action = "impounded" })
                    else
                        HelpText(string.format(Strings["browsing_vehicles"]))
                    end
                end
                    ...
                    for k, v in pairs(Cache.NearbyGarages) do
                        ...
                        if v.location then
                            ...
                            if PlayerData.job.name == v.specifiedJob then
                                ...
                                elseif Marker(v.browse.xyz + vec3(0.0, 0.0, 0.8), Strings["enter_frak_garage"]) then
                ...
                if Cache.NearbyImpound then
                    for k, v in pairs(Cache.NearbyImpounds) do
                        if not IsPedInAnyVehicle(PlayerPedId()) and Marker(v.Retrieve + vec3(0.0, 0.0, 0.8), Strings["enter_impound"]) then
                            ViewImpound(v.Retrieve, Config.DefaultType, v.Spawn.xyz, v.Spawn.w)
                        end
                    end
                end
            end
