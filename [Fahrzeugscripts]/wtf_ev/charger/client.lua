...
local function chargingLoop(ped, vehicle, vcoords, cscoords)
    ...
    if level >= 100.0 then
        SIThreads:LoopForDuration('SS_EV_TEXT_ALREADY_CHARGED', 3000, function()
            Citizen.Wait(1)
            DrawText3Ds(cscoords.x, cscoords.y, cscoords.z + 1.0, "Fahrzeug vollständig geladen!")
        end)
        return
    end
    ...
    while true do
        ...
        chargeWait.Wait(100, function()
            ...
            exports["LegacyFuel"]:SetFuel(vehicle, level / 1.0)
        end)
        ...
        DrawText3Ds(cscoords.x, cscoords.y, cscoords.z + 1.0, "Drücke ~g~G ~w~um das Laden zu beenden.")
...
local function runChargingStation(ped, cscoords, stop)
    ...
    if IsPedInAnyEVVehicle(ped) then
        ...
        AddTextComponentString('Verlass das Fahrzeug zum Laden')
    ...
    if vdistance < 3 then
        SetTextComponentFormat('STRING')
        AddTextComponentString('Drücke ~INPUT_PICKUP~ um mit dem Laden zu beginnen')
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)
        ...
        SetTextComponentFormat('STRING')
        AddTextComponentString('Zu weit weg für die Kabel')
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)
    end
end
...
if Config.EnableChargerBlips then
    for _, v in ipairs(Config.SuperchargerStations) do
        ...
        AddTextComponentString("E Säulen")
