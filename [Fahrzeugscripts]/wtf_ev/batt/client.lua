local function runFuelMonitor(stop)
    ...
    while true do
        ...
        -- draw hud
        local level = exports["LegacyFuel"]:GetFuel(vehicle)
        local r, g, b = GetGreenToRedRGB(level/100.0)
        DrawTxt(UI.x + 0.92, UI.y + 1.444, 1.0,1.0,0.50, string.format('%.1f %%', level), r, g, b, 255)
        ...
        inlineWait.Wait(Config.LevelSamples, function()
            ...
            if level > 0 then
                ...
                totalRPM = 0
                local avgEnergy
                if avgrpm <= 0.25 then avgEnergy = 0.01 -- don't penalize idle
                elseif avgrpm <= 0.35 then avgEnergy = avgrpm * 0.05
                elseif avgrpm <= 0.55 then avgEnergy = avgrpm * 0.10
                elseif avgrpm <= 0.70 then avgEnergy = avgrpm * 0.15
                elseif avgrpm <= 0.80 then avgEnergy = avgrpm * 0.25
