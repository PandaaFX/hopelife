							...
							keyItem.OnClick = function()
								if (not TableContains(Config.modelBlacklist, key[4])) then
									...
									if (type(result) == "boolean" and result == true) then
										Notification(string.format(Config.Strings.createSuccess, key[1]))
									...
									elseif (type(result) == "string" and result == "noMoney") then
										Notification(string.format(Config.Strings.createNoMoney, key[1]))
									else
										Notification(string.format(Config.Strings.createFailed, key[1]))
									end
								else
									Notification(string.format(Config.Strings.createFailed, key[1]))
								end
					...
					for i, vehicle in ipairs(vehicleData) do
						...
						keyItem.OnClick = function()
							if (not TableContains(Config.modelBlacklist, vehicle[2])) then
								...
								if (type(result) == "boolean" and result == true) then
									Notification(string.format(Config.Strings.createSuccess, model .. " " .. plate))

									local oldCount = keyItem.rightText.title:gsub("x", "")
									keyItem.rightText.title = "x" .. tostring(tonumber(oldCount) + 1)
								elseif (type(result) == "string" and result == "noMoney") then
									Notification(string.format(Config.Strings.createNoMoney, model .. " " .. plate))
								else
									Notification(string.format(Config.Strings.createFailed, model .. " " .. plate))
								end
							else
								Notification(string.format(Config.Strings.createFailed, model .. " " .. plate))
							end
						...
						for i, key in ipairs(keys) do
							...
							item.OnClick = function()
								if (not TableContains(Config.modelBlacklist, key[3])) then
									...
									if (type(result) == "boolean" and result == true) then
										Notification(string.format(Config.Strings.deleteKeys, key[1]))
									elseif (type(result) == "string" and result == "noMoney") then
										Notification(string.format(Config.Strings.removeNoMoney, key[1]))
									else
										Notification(string.format(Config.Strings.removeFailed, key[1]))
									end
								else
									Notification(string.format(Config.Strings.removeFailed, key[1]))
								end
					...
					for i, vehicle in ipairs(vehicleData) do
						...
						item.OnClick = function()
							if (not TableContains(Config.modelBlacklist, vehicle[2])) then
								...
								if (type(result) == "boolean" and result == true) then
									Notification(string.format(Config.Strings.deleteKeys, model .. " " .. plate))
								elseif (type(result) == "string" and result == "noMoney") then
									Notification(string.format(Config.Strings.removeNoMoney, model .. " " .. plate))
								else
									Notification(string.format(Config.Strings.removeFailed, model .. " " .. plate))
								end
							else
								Notification(string.format(Config.Strings.removeFailed, model .. " " .. plate))
							end
									...
									if (not TableContains(Config.modelBlacklist, key[3])) then
										...
										for i, player in ipairs(players) do
											if (GetPlayerPed(player) == hitEntity) then
												...
												if (success) then
													Notification(string.format(Config.Strings.giveMasterSuccess, key[1]))

													menuPool:CloseAllMenus()
												else
													Notification(string.format(Config.Strings.giveMasterFailed, key[1]))
												endS
								...
								if (not TableContains(Config.modelBlacklist, vehicle[2])) then
									...
									for i, player in ipairs(players) do
										if (GetPlayerPed(player) == hitEntity) then
											...
											--if (success) then
											--	Notification(string.format(Config.Strings.giveMasterSuccess, model .. " " .. plate))
											--else
											--	Notification(string.format(Config.Strings.giveMasterFailed, model .. " " .. plate))
											--end
								...
								if (not TableContains(Config.modelBlacklist, key[4])) then
									...
									for i, player in ipairs(players) do
										if (GetPlayerPed(player) == hitEntity) then
											...
											if (success) then
												Notification(string.format(Config.Strings.giveSuccess, key[1]))
											else
												Notification(string.format(Config.Strings.giveFailed, key[1]))
											end
							...
							if (not TableContains(Config.modelBlacklist, key.model)) then
								...
								for i, player in ipairs(players) do
									if (GetPlayerPed(player) == hitEntity) then
										...
										if (success) then
											Notification(string.format(Config.Strings.giveSuccess, model .. " " .. key.plate))
										else
											Notification(string.format(Config.Strings.giveFailed, model .. " " .. key.plate))
										end
	...
	submenuInvalidateKey.OnItemSelect = function(menu, item, index)
		...
		if (not TableContains(Config.modelBlacklist, vehicleData[index][2])) then
			Citizen.CreateThread(function()
				...
				elseif (type(result) == "string" and result == "noMoney") then
					Notification(string.format(Config.Strings.removeNoMoney, model))
				else
					Notification(string.format(Config.Strings.removeFailed, model))
				end
			end)
		else
			Notification(string.format(Config.Strings.removeFailed, model))
