...
local output = nil
local gasstationname = nil
RegisterNetEvent('LegacyFuel:PandaaFXCB01')
AddEventHandler('LegacyFuel:PandaaFXCB01', function(ret)
    output = ret
end)

RegisterNetEvent('LegacyFuel:PandaaFXCB02')
AddEventHandler('LegacyFuel:PandaaFXCB02', function(ret)
	print("CB02")
...
function open(vehicle,data)
	...
	SendNUIMessage({ action = true, fuel = GetVehicleFuelLevel(vehicle), vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)), vehdisplayname = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))), plate = GetVehicleNumberPlateText(vehicle), data = data })
end
...
RegisterNUICallback('pay', function(data, cb)
	...
	if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(vehicle)) < 5 then
		TriggerServerEvent('LegacyFuel:pagamento01',math.round(new_perc),false,VehToNet(vehicle),math.round(new_perc),Config.FuelDecor,pumpLocation)
	end
end)
...
if Config.EnableToxicPumpNozzles then
	...
	AddEventHandler('fuel:refuelFromPump', function(ped, vehicle)
		output = nil
		TriggerServerEvent('LegacyFuel:PandaaFXCB01',pumpLocation)
		while output == nil do 
			Wait(10)
		end
...
AddEventHandler(eventName, function(ped, vehicle)
	...
	output = nil
	TriggerServerEvent('LegacyFuel:PandaaFXSB02')
	while output == nil do 
		Wait(10)
	end
	if output then
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				elseif isNearPump then
					...
					if hasJerryCan then
						-- print(GetAmmoInPedWeapon(ped, 883325847))
						if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.EToRefillJerryCan)

							if IsControlJustReleased(0, 38) then
								TriggerServerEvent('LegacyFuel:pagamento01',Config.RefillCost,true)
							end
						else
