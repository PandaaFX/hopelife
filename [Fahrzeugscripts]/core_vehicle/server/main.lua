    ...
    function(partType, part)
        ...
        local xPlayer = ESX.GetPlayerFromId(src)
         ...
         if Config.UsingCoreInventory then
             ...
             local xItems = exports['core_inventory']:getItems('content-' .. xPlayer.getIdentifier():gsub(":", ""), part)

        if xItems[1] then
        if xItems[1].count > 0 then
            xPlayer.removeInventoryItem(part, 1)
            TriggerClientEvent("core_vehicle:startInstall", src, partType, part, xItems[1].metadata.health or 100.0)
        else
        ...
        if xItem.count > 0 then
            xPlayer.removeInventoryItem(part, 1)
            TriggerClientEvent("core_vehicle:startInstall", src, partType, part, 100.0)
        else
            TriggerClientEvent("core_vehicle:SendTextMessage", src, Config.Text["not_enough"])
        end
        end
...
RegisterServerEvent("core_vehicle:removePart")
AddEventHandler(
    "core_vehicle:removePart",
    function(part, health)

        if Config.UsingCoreInventory then
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)

       xPlayer.addInventoryItem(part, 1, {health = health})
        end
    end
)
