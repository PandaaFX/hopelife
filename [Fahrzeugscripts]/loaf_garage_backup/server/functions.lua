...
GetVehicles = function(source, garage, cb)
    ...
    if Config.Garages[garage] and Config.Garages[garage].vehicletype or Config.Garages[garage].vehicletype2 or Config.Garages[garage].vehicletype3 then
        ...
        MySQL.Async.fetchAll("SELECT `vehicle`, `type`, `damages`, `garage`, `job`, `plate`, `stored` FROM `owned_vehicles` WHERE `owner` = @identifier", {
            ...
            if result and #result > 0 then
                ...
                for k, v in pairs(result) do
                    ...
                    if (v.job == Config.DefaultJob or Config.ShowJobVehicles) and v.type == vehicletype or v.type == vehicletype2 or v.type == vehicletype3 then
