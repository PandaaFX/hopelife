...
$(document).on('click', '#phone-enter-deletelast', function () {
  ...
  $('#phone-number-enter').html(
    $('#phone-number-enter').text().replace(/.$/g, '')
  );
  ...
  if (mutes == true) {
    if (locale == 'de') {
      sendData('notification', {
        text: localede.somebodyiscallingyou,
        length: 5000,
      });
    } else {
      sendData('notification', {
        text: locale.somebodyiscallingyou,
        length: 5000,
      });
    }
  } else {
