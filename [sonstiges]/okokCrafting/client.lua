...
local itemAmnt, timeCraft, itemRecipe, craftss, success, isItem, xp
...
Citizen.CreateThread(function()
	...
	while true do
			...
			if distance < v.radius + 2 then
				...
				if distance < v.radius then
					...
					if not Config.UseOkokTextUI and not isCraftOpen then
						ESX.ShowHelpNotification('Drücke ~INPUT_CONTEXT~ um mit der Herstellung zu beginnen')
					end
					if IsControlJustReleased(0, Config.Key) then
						if GetVehiclePedIsUsing(PlayerPedId()) == 0 then
							...
							if not isCraftOpen then
								...
								ESX.TriggerServerCallback("okokCrafting:itemNames", function(itemNames)
									...
									if not Config.UseXP then
										...
										ESX.TriggerServerCallback("okokCrafting:getLevel", function(level, percentage)
											SetNuiFocus(true, true)
											SendNUIMessage({
												action = "openCraft",
												name = v.tableName,
							...
							exports['okokNotify']:Alert("CRAFTING", "Du kannst nicht in einem Fahrzeug etwas Herstellen", 5000, 'error')
						end
					end
				end
		...
		if inZone and not hasEntered then
			if Config.UseOkokTextUI then
				exports['okokTextUI']:Open('Drücke [E] um mit der Herstellung zu beginnen', 'darkblue', 'left') 
			end
			hasEntered = true
