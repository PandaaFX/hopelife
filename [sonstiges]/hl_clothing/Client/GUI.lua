...
Citizen.CreateThread(function()
	...
	while true do Wait(0)
		if not Config.GUI.Toggle then
			...
			if IsControlPressed(1, Config.GUI.Key) and not IsPedInAnyVehicle(Ped, true) then
				if Check(Ped) then MenuOpened = true end
			else MenuOpened = false end
			if IsControlJustPressed(1, Config.GUI.Key) and not IsPedInAnyVehicle(Ped, true) then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) end
			elseif IsControlJustReleased(1, Config.GUI.Key) and not IsPedInAnyVehicle(Ped, true) then
