...
window.addEventListener('message', function(event) {
	switch (event.data.action) {
			...
			if (selectedWindow == "none") {
				...
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div class="ldata-txt">Daten werden geladen...</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
					...
					$('#menu').html(`
						...
						<div class="col-md-10 tab-s">
							<div class="d-flex justify-content-between align-items-center">
								<span class="selected-page"><span id="page-title">Übersicht</span></span>
								<div>
		...
		case 'transactions_page':
			...
			$('#page-title').html('Transaktionen');

			if (event.data.isInSociety){
				society = `<span class="sidebar-title mt-5">Society</span>
						   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
						   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>`;
			} else {
				society = '';
			}

			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
				<p class="sidebar-item selected" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>
				<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Einstellungen</span></p>
				${society}
			`);
			...
			for(var i = 0; i < num; i++) {
				...
				if (db.type == 'transfer' && db.receiver_identifier == event.data.identifier) {
					...
					data = `<td class="align-middle fw500">
								...
								<div class="mtm3125">Erhalten</div>
							</td>`;
				...
				// Gesendet
					...
					data = `<td class="align-middle fw500">
								...
								<div class="mtm3125">Gesendet</div>
							</td>`;
				...
				// Eingezahlt
					...
					data = `<td class="align-middle fw500">
								...
								<div class="mtm3125">Abgehoben</div>
							</td>`;
					amount = `<td class="align-middle fw500 transactions-received text-center">+ ${db.value.toLocaleString()} $</td>`;
				// Ausgezahlt
					...
					data = `<td class="align-middle fw500">
								From <span class="transactions-name">${name}</span>
								<div class="mtm3125">Eingezahlt</div>
							</td>`;
					amount = `<td class="align-middle fw500 text-center">- ${db.value.toLocaleString()} $</td>`;
				}
			...
			$('#page_info').html(`
				<div class="row mb-3">
					<div class="col col-md-3 pr05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">TRANSAKTIONEN</h6>
					...
					<div class="col col-md-3 pr05 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">EINKOMMEN</h6>
					...
					<div class="col col-md-3 pr05 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">AUSGABEN</h6>
		...
		case 'society_transactions':
			...
			$('#page-title').html('Transaktionen ['+event.data.societyInfo.society_name+']');

			if (event.data.isInSociety){
				society = `<span class="sidebar-title mt-5">Society</span>
						   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
						   <p class="sidebar-item selected" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>`;
			} else {
				society = '';
			}
			...
			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
				<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>
				<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Einstellungen</span></p>
				${society}
			`);
			...
			$('#page_info').html(`
				<div class="row mb-3">
					...
					<div class="col col-md-3 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">Einnahmen</h6>
								<p class="card-text fw125" id="awaitedIncome">${event.data.graph_values[9].toLocaleString()}$</p>
							</div>
						</div>
					</div>
...
function overview_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "bankmenu" || !event.data.isUpdate){
		...
		$('#sidebar').html(`
			<p class="sidebar-item mt-2 selected" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
			<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>
			<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Einstellungen</span></p>
			${society}
		`);
		...
		if(event.data.RequireCC) {
			informations = `<span class="card-o-title">Informationen<span class="badge bg-primary viewall-badge buy_new_card" data-bs-toggle="modal" data-bs-target="#buycc_modal"><i class="fa-solid fa-plus"></i> NEU</span></span>`;
		} else {
			informations = `<span class="card-o-title">Informationen</span>`;
		}
					...
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Letzte Transaktionen<span class="badge bg-primary viewall-badge" id="view_all_transactions"><i class="fas fa-eye"></i> ALLE ANSEHEN</span></span>
							</div>
							...
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
									...
									<div class="mt38p">
										...
										<div class="d-flex justify-content-between align-items-center">
											<span class="ccard-active">AKTIV</span>
											<div class="d-flex align-items-center ccard-valid">
												<span class="ccard-thru">GÜLTIG BIS</span>
					...
					<div class="card text-center h-100 actions-card">
						...
						<div class="card-body actions-card_body">
							<div class="d-flex justify-content-center">
								<button type="button" id="depositMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#depositModal"><i class="bi bi-upload"></i> Einzahlung</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
								<button type="button" id="withdrawMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#withdrawModal"><i class="bi bi-download"></i> Auszahlung</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
		...
		for(var i = 0; i < num; i++) {
			...
			// Ausgezahlt
				...
				data = `<td class="align-middle lastT-name-div">
							From <span class="transactions-name">${name}</span>
							<div class="mtm3125">Ausgezahlt</div>
						</td>`;
				amount = `<td class="align-middle fw500"><span class="floatr">- ${db.value.toLocaleString()} $</span></td>`;
			}
...
function society_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "societies" || !event.data.isUpdate){
		...
		$('#page-title').html('Übersicht ['+event.data.societyInfo.society_name+']');

		if (event.data.isInSociety){
			society = `<span class="sidebar-title mt-5">Society</span>
					   <p class="sidebar-item mt-2 selected" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
					   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>`;
		} else {
			society = '';
		}
		...
		$('#page_info').html(`
			<div class="col-md-7 chart-main">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Letzte Transaktionen<span class="badge bg-primary viewall-badge" id="view_all_transactions_society"><i class="fas fa-eye"></i> VIEW ALL</span></span>
							</div>
							<div class="card-body" id="last-t-body">
								<table id="lastTransactionsTable">
									<tbody id="lastTransactionsData">
			...
			<div class="col-md-5">
				<div class="col-md-12">
					<div class="card ccard-card">
						<div class="card-header card-o-header text-center">
							<span class="card-o-title">Informationen</span>
						</div>
						<div class="card-body ccard-body">
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
...
function settings_page_function(event) {
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Übersicht</span></p>
		<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transaktionen</span></p>
		<p class="sidebar-item mt-2 selected" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Einstellungen</span></p>
		${society}
	`);
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changeiban-card">
									<div class="card-body text-center">
										<span class="fs15 fff">Ändere deine IBAN</span>
										<input type="text" maxlength="${event.data.ibanCharNum}" id="new_iban" class="form-control text-center" placeholder="NEUE IBAN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_iban" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Abschicken</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Die IBAN hat Änderungskosten in Höhe von ${event.data.ibanCost}$</span>
										<hr class="fff">
										<span>Die IBAN hat immer den Präfix "${event.data.ibanPrefix}"</span>
										<hr class="fff">
										<span>Die maximale Anzahl von Zeichen beträgt ${event.data.ibanCharNum}</span>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changepin-card">
									<div class="card-body text-center">
										<span class="fs15 fff">Ändere deinen PIN</span>
										<input type="password" maxlength="4" id="new_pin" class="form-control text-center" placeholder="NEUE PIN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_pin" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Abschicken</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Die PIN hat Änderungskosten in Höhe von ${event.data.pinCost}$</span>
										<hr class="fff">
										<span>Die maximale Anzahl von Zeichen beträgt ${event.data.pinCharNum}</span>
										<hr class="fff">
										<span>Du kannst nur Zahlen verwenden</span>
									</div>
								</div>
							</div>
						</div>
	...
	setTimeout(function () {
		$('#menu').html(`
			<div class="d-flex justify-content-center flex-column align-items-center">
				<span class="load"></span>
				<br>
				<div class="ldata-txt">Abmeldung...</div>
			</div>
		`);
		$("#menu").fadeIn();
