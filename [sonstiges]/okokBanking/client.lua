	...
	while true do
		...
		if NearBank() and not isBankOpened and NearBank() ~= "update" then
			if not Config.okokTextUI then
				ESX.ShowHelpNotification("Drücke ~INPUT_PICKUP~ um die ~b~Bank aufzurufen")
			else
				inRange = true
			end
			...
			if IsControlJustReleased(0, 38) then
				...
				if not isDead and not IsPedInAnyVehicle(playerped) then
					...
					exports['okokNotify']:Alert("ATM", _U("not_use_bank"), 3000, 'error')
						notified = true
					end
				end
			end
		...
		if inRange and not shown then
			shown = true
			exports['okokTextUI']:Open('Drücke [E] um die Bank aufzurufen', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if NearATM() and not isBankOpened and NearATM() ~= "update" then
			if not Config.okokTextUI then
				ESX.ShowHelpNotification("Drücke ~INPUT_PICKUP~, um den ~b~ATM zu öffnen")
			else
				inRange = true
			end
				...
				if not isDead and not IsPedInAnyVehicle(ped) then
					if Config.RequireCreditCardForATM then
						ESX.TriggerServerCallback("okokBanking:HasCreditCard", function(hasItem)
							if not hasItem then
								exports['okokNotify']:Alert("ATM", _U("no_creditcard"), 5000, 'error')
								return
							else
								ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
									if pin then
										...
										exports['okokNotify']:Alert("ATM", _U("no_pin"), 5000, 'warning')
									end
								end)
							end
						end)
		...
		if inRange and not shown then
			shown = true
			exports['okokTextUI']:Open('Drücke [E], um den ATM zu öffnen', 'darkblue', 'left') 
		elseif not inRange and shown then
			shown = false
			exports['okokTextUI']:Close()
		end
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['okokNotify']:Alert("BANK", _U("invalid_amount"), 5000, 'error')
			end
		else
			exports['okokNotify']:Alert("BANK", _U("invalid_amount"), 5000, 'error')
		end
	elseif data.action == "withdraw" then
		if tonumber(data.value) ~= nil then
			...
			exports['okokNotify']:Alert("BANK", _U("invalid_input"), 5000, 'error')
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						exports['okokNotify']:Alert("BANK", _U("iban_not_exist"), 5000, 'error') _U("invalid")
					end
				end, data.iban:upper())
			else
				exports['okokNotify']:Alert("BANK", _U("invalid_amount"), 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
						exports['okokNotify']:Alert("BANK", _U("iban_in_use"), 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['okokNotify']:Alert("BANK", _U("iban_only_numbers"), 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['okokNotify']:Alert("BANK", _U("pin_digits"), 5000, 'info')
			end
		else
			exports['okokNotify']:Alert("BANK", _U("pin_only_numbers"), 5000, 'info')
		end
