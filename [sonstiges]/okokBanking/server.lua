...
local Webhook = 'https://discord.com/api/webhooks/981107001708929034/hxth8V1xjGyDnvVqq4iJF6aR5RPbIZfa-aiVa_e16Sly-v81sr3WAyQTuustCxQJav3g'
...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("deposited", amount), 5000, 'success')
		DepositMoneyWebhook({sender_name = getName(source), value = ESX.Math.GroupDigits(amount)})
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("no_money_pocket"), 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("withdrawn", amount), 5000, 'success')
		WithdrawMoneyWebhook({receiver_name = getName(source), value = ESX.Math.GroupDigits(amount)})
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("no_money_bank"), 5000, 'error')
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
					...
					if xForPlayer.identifier == targetIdentifier then
						...
						TriggerClientEvent('okokNotify:Alert', xPlayers[i], "BANK", _U("received_from", amount, getName(_source)), 5000, 'success')
					end
				end
				...
				TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("transferred_to", amount, getName(targetIdentifier)), 5000, 'success')
			...
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', _source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("transferred_to", amount, tName), 5000, 'success')
			...
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("no_money_bank"), 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("not_send_yourself"), 5000, 'success')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("deposited_to", amount, societyName), 5000, 'success')
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if db.value ~= nil and amount <= db.value or db.money ~= nil and amount <= db.money then
		if db.is_withdrawing ~= nil and db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("someone_withdrawing"), 5000, 'success')
		else
			if Config.UseAddonAccount then
				MySQL.Async.execute('UPDATE addon_account_data SET money = money - @money WHERE account_name = @account_name', {
					['@money'] = amount,
			...
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("you_have_withdrawn", amount, societyName), 5000, 'success')
			TransferMoneyWebhook({sender_name = societyName..' ('..society..')', receiver_name = getName(source), value = amount})
		end
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("society_no_money"), 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("transferred_to", amount, societyName), 5000, 'success')
		TransferMoneyWebhook({sender_name = getName(_source), receiver_name = societyName..' ('..society..')', value = amount})
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("no_money_bank"), 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("transferred_to", amount, societyNameTarget), 5000, 'success')
		TransferMoneyWebhook({sender_name = societyName..' ('..society..')', receiver_name = societyNameTarget..' ('..societyTarget..')', value = amount})
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("society_no_money"), 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		if xTarget ~= nil then
			...
			if not toMyself then
				for i=1, #xPlayers, 1 do
					...
					if xForPlayer.identifier == targetIdentifier then
						TriggerClientEvent('okokBanking:updateTransactions', xPlayers[i], xTarget.getAccount('bank').money, xTarget.getMoney())
						TriggerClientEvent('okokNotify:Alert', xPlayers[i], "BANK", _U("received_from", amount, getName(source)), 5000, 'success')
					end
				end
			end
			TriggerEvent('okokBanking:AddTransferTransactionFromSocietyToP', amount, society, societyName, targetIdentifier, targetName)
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("transferred_to", amount, getName(targetIdentifier)), 5000, 'success')
		...
		elseif xTarget == nil then
			...
			TriggerEvent('okokBanking:AddTransferTransactionFromSocietyToP', amount, society, societyName, targetIdentifier, targetName)
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("transferred_to", amount, targetName), 5000, 'success')
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("iban_changed", iban), 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK",_U("iban_no_money", amount), 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', _source)
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("pin_changed", pin), 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", _U("pin_no_money", amount), 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:GiveCC", function()
	...
	if Config.CreditCardPrice <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', xPlayer.source, "BANK", _U("bought_cc", Config.CreditCardPrice), 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", _U("no_money_bank"), 5000, 'error')
	end
end)
