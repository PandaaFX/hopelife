...
local function SlowTimer()
    ...
    -- redraw player chips like a boss
    local inv = GetClientSideInventory()
    PLAYER_CHIPS = inv[Config.ChipsInventoryItem] or 0
...
function GetClientSideInventory()
    local items = {}
    if Framework.Active == 1 then
        local data = ESX.GetPlayerData()
        if data then
            for k, v in ipairs(data.inventory or {}) do
                if v.count > 0 then
    ...
    elseif Framework.Active == 2 then
        ...
        if data then
            for k, v in ipairs(data.items or {}) do
                if v.amount > 0 then
