...
AddEventHandler('hl:revive', function(playerId)
			...
			if xTarget then
				if deadPlayers[playerId] then
					if Config.ReviveReward > 0 then
						...
						xTarget.triggerEvent('hl:revive')
...
AddEventHandler('txAdmin:events:healedPlayer', function(eventData)
	...
	if deadPlayers[eventData.id] then
		TriggerClientEvent('hl:revive', eventData.id)
	end
end)
...
RegisterNetEvent('hl:heal')
AddEventHandler('hl:heal', function(target, type)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'ambulance' then
		TriggerClientEvent('hl:heal', target, type)
	end
end)
...
ESX.RegisterCommand('revive', {'admin', 'dev', 'mod', 'support'}, function(xPlayer, args, showError)
	args.playerId.triggerEvent('hl:revive')
...
ESX.RegisterCommand('reviveall', "admin", function(xPlayer, args, showError)
	TriggerClientEvent('hl:revive', -1)
end, false)
