...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		ESX.ShowNotification("Du musst einen Charakter erstellen!")
	end
end)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "dateofbirth" then
			if value == "invalid" then
				reason = "Ungültiger Geburtsdatum!"
				break
			end
		...
		elseif theData == "sex" then
			if value == "invalid" then
				reason = "Du musst Mann oder Frau sein! (M/F)"
				break
			end
		end
	end
...
function verifyName(name)
	...
	if count ~= nameLength then
		return 'Ihr Spielername enthält Sonderzeichen, die auf diesem Server nicht erlaubt sind.'
	end
