...
window.addEventListener('message', function(event) {
	...
	case 'openSideCraft':
			...
			if (event.data.recipe[i][2]){
				recipe += `
					<div class="d-flex align-items-center mx-1">
						...
						<span style="color: #fff; font-weight: 600; font-size: 20px;" class="ms-2">${capitalizeTxt(idName)} x${event.data.recipe[i][1]} (${event.data.inventory[i].key})</span>
					</div>
				`;
			} else {
				recipe += `
		...
		if(canCraft) {
			$('#craft-button-div').html(`
				<button type="button" id="craft-button" data-item="${event.data.itemNameID}" data-recipe="${event.data.recipe}" data-amount="${event.data.itemAmount}" data-xp="${event.data.xp}" onclick="craft(this)" class="btn btn-blue flex-grow-1 mt-2" style="border-radius: 10px; flex-basis: 100%; width: 100%; font-weight: 600;"><i class="fas fa-pencil-ruler"></i> HERSTELLEN [${event.data.itemName}]</button>
			`);
			$('#craft-button').fadeIn();
		} else {
	...
	case 'ShowCraftCount':
		...
		$('#crafting-text').html(`Herstelllen`);
	...
	case 'CompleteCraftCount':
		...
		$('#crafting-text').html(`Hergestellt`);
		$('#item-name').html(`${itemName}`);
		$('#item-timer').html(`0s`);
	...
	case 'FailedCraftCount':
		...
		$('#crafting-text').html(`Fehlgeschlagene Herstellung`);
		$('#item-name').html(`${itemName}`);
		$('#item-timer').html(`0s`);
