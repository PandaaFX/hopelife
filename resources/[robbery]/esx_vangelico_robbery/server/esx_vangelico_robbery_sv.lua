...
function CountCops()
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' or xPlayer.job.name == 'lssd' or xPlayer.job.name == 'fib' then
...
AddEventHandler('esx_vangelico_robbery:rob', function(robb)
	...
	if Stores[robb] then
			...
			local embed = {
				{
					["color"] = "000000",
					["title"] = "**" .. playerName .. "**",
					["description"] = "Started Vangelico Robbery",
				}
			}
		...
		PerformHttpRequest(Config.webhook, function (err, text, headers) end, 'POST', json.encode({username = name, embeds = connecting}), { ['Content-Type'] = 'application/json' })
