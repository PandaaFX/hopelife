...
-- Overall Info
RegisterNetEvent('hl_hud:setInfo')
AddEventHandler('hl_hud:setInfo', function()
    ...
    for i = 1, #data.accounts do 
		...
		elseif data.accounts[i].name == 'black_money' then
            black = data.accounts[i].money
        end
    end
	SendNUIMessage({ action = 'setMoney', id = 'moneyP', value = money })
	SendNUIMessage({ action = 'setMoney', id = 'bank_cardP', value = bank })
	if(black > 0) then
		if not toggleui then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'blackMoney' })
		end
		SendNUIMessage({ action = 'setMoney', id = 'black_moneyP', value = black })
	else 
	...
	SendNUIMessage({ action = 'playerID', id = 'idP', value = id })
end)
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'anderer_wrapper'})
	else
		...
		SendNUIMessage({ action = 'element', task = 'enable', value = 'anderer_wrapper'})
	end
