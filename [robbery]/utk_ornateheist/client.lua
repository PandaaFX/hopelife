...
UTK = {
    ...
    text = { -- Texts
        ...
        silentstart = "[~r~E~w~] um die Tür zu ~g~knacken~w~",
        usecard = "[~r~E~w~] ID Karte benutzen",
        usethermal = "[~r~E~w~] Thermal Ladung benutzen",
        usehack = "[~r~E~w~] Security Panel hacken",
        uselockpick = "[~r~E~w~] Dietrich benutzen",
        ...
        search = "suche...",
        hacking = "hacke...",
        melted = "Türschloss geschmolzen",
        hacked = "Hack abgeschlossen",
        unlocked = "Tür offen",
        nothing = "Nichts gefunden.",
        found = "Du hast die ID Karte gefunden",
        time = "Zeit übrig: "
    }
}
UTK.searchinfo = {
                ...
                while true do
                    local coords = GetEntityCoords(PlayerPedId())
                        ...
                        if dst <= 6 then
                            ...
                            --local requiredJob = "police"
                            --local requiredJob2 = "fib"
                            --local requiredJob3 = "lssd"
                            if dst <= 1 and IsControlJustReleased(0,38) then
                                ESX.TriggerServerCallback("utk_oh:startevent", function(output)
                                    if output == true then
                                        ...
                                        self.currentplant = 0
                                        self:Plant()
                                        --TriggerServerEvent('roadphone:sendDispatch', GetPlayerServerId(PlayerId()), "Ein Raub ist im Gange", requiredJob, {x  = 257.10, y = 220.30, z = 106.28}, true)
...
function UTK:Plant()
    ...
    Citizen.Wait(9000)
    TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Türe offen!', 5000)
...
function UTK:Lockpick()
    ...
    Citizen.Wait(6500)
    TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Schlossknacken gestartet', 5000)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    exports['progressBars']:startUI(6000, self.text.lockpick..": Stage 2")
    Citizen.Wait(6500)
    TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Schlossknacken erfolgreich', 5000)
    TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Schlossknacken erfolgreich', 5000)
    UTK.disableinput = false
    if self.currentpick == 0 then
        UTK.stage0break = true
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Öffne Mein Computer - Externes Device - dann Brute Force', 5000)
...
function UTK:IdCard()
    ...
    if self.currentid == 1 then
        UTK.disableinput = false
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Tür offen!', 5000)
    ...
    elseif self.currentid == 2 then
        ...
        repeat
            ...
            count = count - 1
        until count == 0
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Erfolgreich!', 5000)
        SpawnObj()
...
function Search(location)
    ...
    if UTK.searchinfo.random ~= 1 then
        ...
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'Nichts gefunden', 5000)
        UTK.searchinfo.random = math.random(1, UTK.cur - 1)
        UTK.cur = UTK.cur - 1
    else
        UTK.searchinfo.found = true
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'ACHTUNG', 'ID Card gefunden', 5000)
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if IsScaleformMovieMethodReturnValueReady(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    TriggerEvent('hopelife:notify', 1, '#5f0085', 'info', 'Suche die IP Adresse!', 5000)
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    TriggerEvent('hopelife:notify', 1, '#5f0085', 'info', 'Finde das Passwort!', 5000)
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    TriggerEvent('hopelife:notify', 1, '#5f0085', 'info', 'Bruteforce.exe initialisieren!', 5000)
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        TriggerEvent('hopelife:notify', 1, '#5f0085', 'success', UTK.text.hacked, 5000)
                        UsingComputer = false
                        UTK.disableinput = false
                        UTK.hackfinish = true
                        Ipfinished = false
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['esx_notify']:SendAlert("error", "You have 90 seconds until toxic gas comes in.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            TriggerEvent('hopelife:notify', 1, '#5f0085', 'success', "Du raubst " ..text, 5000)
        end)
    else
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'error', "Du hast 90 Sek. bevor Gas einströmt", 5000)
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        TriggerEvent('hopelife:notify', 1, '#5f0085', 'error', "Die Tresortür schließt sich in 60 Sek.", 5000)
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                TriggerEvent('hopelife:notify', 1, '#5f0085', 'error', "Tresortür geschlossen!", 5000)
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" or player.job.name == "lssd" or police.job.name == "fib" then
        if toggle == 1 then
            TriggerEvent('hopelife:notify', 1, '#5f0085', 'info', "Pacific Bank Alarm aktiert", 10000)
