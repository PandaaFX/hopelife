RegisterNetEvent("antivehiclekick:enteringVehicle")
AddEventHandler('antivehiclekick:enteringVehicle', function (targetVehicle, vehicleSeat, vehicleDisplayName)
    local playerPed = PlayerPedId()
    -- print(targetVehicle, vehicleSeat, vehicleDisplayName)
    if IsVehicleSeatFree(targetVehicle, vehicleSeat) then
        -- TaskEnterVehicle(playerPed, targetVehicle, 10000, vehicleSeat, 1.0, 1, 0)
    else
...
Citizen.CreateThread(function()
	...
	tiempo = 800
		...
		if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and IsPedAPlayer(GetPedInVehicleSeat(vehicle, -1)) or DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and GetPedInVehicleSeat(vehicle, -1) == 0 then
			...
			local distanceToBrdoor = #(playerpos - brdoor)
			local distanceToBldoor = #(playerpos - bldoor)
			local distanceToFrdoor = #(playerpos - frdoor)



				-- print(distanceToBrdoor, distanceToBldoor, distanceToFrdoor)
		    if distanceToBrdoor < 0.8 and DoesVehicleHaveDoor(vehicle, 3) and not DoesEntityExist(GetPedInVehicleSeat(vehicle, 2)) and GetVehicleDoorLockStatus(vehicle) ~= 2 then
			...
			elseif distanceToBldoor < 0.8 and DoesVehicleHaveDoor(vehicle, 2) and not DoesEntityExist(GetPedInVehicleSeat(vehicle, 1)) and GetVehicleDoorLockStatus(vehicle) ~= 2 then
                DrawText3D(bldoor.x, bldoor.y, bldoor.z + 0.3, "⬆️")
				if (IsControlJustPressed(1, 49)) then
                 TaskEnterVehicle(PlayerPedId(), vehicle, 10000, 1, 1.0, 1, 0)
                end
			elseif distanceToFrdoor < 0.8 and DoesVehicleHaveDoor(vehicle, 1) and not DoesEntityExist(GetPedInVehicleSeat(vehicle, 0)) and GetVehicleDoorLockStatus(vehicle) ~= 2 then
