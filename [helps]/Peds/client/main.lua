...
-- Nightclub Girls1
Citizen.CreateThread(function()
    RequestModel(GetHashKey("ig_tracydisanto"))
	
    while not HasModelLoaded(GetHashKey("ig_tracydisanto")) do
        Wait(1)
    end
	
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations2) do
			local npc = CreatePed(4, 0xDE352A35, item.x, item.y, item.z, item.heading, false, true)
	        ...
	        Citizen.Wait(200)	
			TaskPlayAnim(npc,"mini@strip_club@private_dance@part1","priv_dance_p1",1.0, 1.0, -1, 9, 1.0, 0, 0, 0)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations3) do
	        ...
	        Citizen.Wait(200)
			TaskPlayAnim(npc,"mini@strip_club@private_dance@part2","priv_dance_p2",1.0, 1.0, -1, 9, 1.0, 0, 0, 0)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableNightclubs then
		for _, item in pairs(Config.Locations4) do
	        ...
	        Citizen.Wait(200)
			TaskPlayAnim(npc,"mini@strip_club@private_dance@part3","priv_dance_p3",1.0, 1.0, -1, 9, 1.0, 0, 0, 0)
		end
	end
end)
