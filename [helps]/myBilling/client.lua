...
function createBillsMenu()
	...
	ESX.TriggerServerCallback('myBilling:getJobname', function(jobname)
		local canGiveBills = false
		if Config.OnlyCertainJobsCanGiveBills then
			for k, jn in pairs(Config.WhitelistedJobs) do
				if jn == jobname then
					canGiveBills = true
					break
				end
		...
		if Config.CanEverybodyGiveBills or canGiveBills then
			...
			submitBill.Activated = function(sender, item)
				--print('isSocietyBill: ' .. tostring(isSocietyBill))
