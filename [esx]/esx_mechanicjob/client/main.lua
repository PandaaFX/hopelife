...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = "Slamtruck",      value = 'dep_vehicle2'}
		...
		elseif data.current.value == 'dep_vehicle2' then
			local playerPed = PlayerPedId()
			local vehicle = GetVehiclePedIsIn(playerPed, true)

			local towmodel = GetHashKey("slamtruck")
			local isVehicleTow = IsVehicleModel(vehicle, towmodel)
			...
			if isVehicleTow then
				...
				if CurrentlyTowedVehicle == nil then
					if targetVehicle ~= 0 then
						if not IsPedInAnyVehicle(playerPed, true) then
							if vehicle ~= targetVehicle then
								AttachEntityToEntity(targetVehicle, vehicle, 20, 0.0, -2.0, 1.0, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
								CurrentlyTowedVehicle = targetVehicle
								ESX.ShowNotification(_U('vehicle_success_attached'))
						...
						ESX.ShowNotification(_U('no_veh_att2'))
					end
				else
					AttachEntityToEntity(CurrentlyTowedVehicle, vehicle, 20, 0.0, -7.0, 1.0, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
					DetachEntity(CurrentlyTowedVehicle, true, true)
...
CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 168) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'outlaws' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, 168) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'soa' then
			OpenMobileMechanicActionsMenu()
		end
--[[
