...
function OpenlssdActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lssd_actions', {
		...
		elements = {
			...
			{label = "Sperrzone", value = 'sperrzone_interaction'},
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = _U('headbag'),value = 'headbag'},
				{label = "Alkoholtest", value = 'alctest'}
			}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'unjail' then
						UnJailPlayer(GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'sperrzone_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				title    = "Sperrzone",
				align    = 'top-left',
				elements = {
					{label = "Sperrzone Eröffnen", value = 'sperrzoneopen'},
					{label = "Sperrzone Schließen", value = 'sperrzoneclose'}

			}}, function(data2, menu2)
				local action = data2.current.value
				if action == 'sperrzoneopen' then
					ExecuteCommand("szon")
				elseif action == 'sperrzoneclose' then
					ExecuteCommand("szoff")
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = "Radarfalle aufbauen",value = 'radar'}
				...
				if action == 'radar' then
					if ESX.PlayerData ~= nil and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name >= "16" then
						TriggerEvent("myRadarcontrol:createRadar")
					else
						ESX.ShowNotification("Nichte Berechtigt dazu")
					end
				end
...
function UnJailPlayer(player)
	TriggerServerEvent("esx_jail:unfromJail", player)
end
