...
RegisterNetEvent('NajblizszyGracz') --This event open menu
AddEventHandler('NajblizszyGracz', function()
  NajblizszyGracz()
end)
...
if closestPlayer == -1 or closestDistance > 2.0 then 
    ESX.ShowNotification('~r~Kein Bürger in der nähe')
else
  if not HaveBagOnHead then
    TriggerServerEvent('esx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    ESX.ShowNotification('~g~Tüte übergezogen ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~Der Bürger hat bereits eine Tüte auf dem Kopf')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Jemand nimmt dir die Tüte ab!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Tüte aufsetzten', value = 'puton'},
          {label = 'Tüte absetzen', value = 'putoff'},
