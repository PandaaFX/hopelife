...
window.addEventListener('message', function (event) {
    ...
    if (item.showmenu) {
        ...
        for (let k of item.vehicleData.servicesNUI) {
            history[c] = {name: k.name, img: k.img, hour: timeConverter(k.timer,item.format.location)+' ('+new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(k.km)+' '+speedometer+')', action: Lang[lang]['service']}
            c++
        }
        ...
        $('#vehicle-km').empty();
        $('#vehicle-km').append(item.vehicleData.plate + ' - ' + new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(item.vehicleData.km/1000) + ' '+speedometer);
		...
		$('.painel').append(zero + new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 2, minimumFractionDigits: 2 }).format(item.km) + ' '+speedometer);
